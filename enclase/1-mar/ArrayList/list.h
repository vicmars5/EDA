...
template <class T, int arraySize>
void List<T, arraySize>::sortDataMerge(int leftEdge, int rightEdge) {
...
template <class T, int arraySize>
void List<T, arraySize>::sortDataQuick(int leftEdge, int rightEdge) {
    if(leftEdge >= rightEdge){
        return;
    }
    ...
    while(i < j) {
        ...
        while(j < i and data[j] >= data[rightEdge]){
            j--;
        }
